...
// 冒泡排序
  ...
  function get_bl($name){
   ...
   function gettypename($type){
	 ...
	 function liushui($mid){
		 global $dosql;
		 $r = $dosql->GetOne("SELECT chargenumber,chargetime FROM pmw_charge where mid=$mid order by id desc limit 0,1");
		 $chargenumber = $r['chargenumber'];   //最后一次充值的金额
		 $chargetime   = $r['chargetime'];       //最后一次充值的时间
		 $Data = array(
			 "money" => $chargenumber,
			 "time" =>  strtotime($chargetime)
		 );
		 return $Data;
	 }
	 ...
	 function xiazhumoney($mid){
		 global $dosql;
		 $sum_money = "";
		 $arr=liushui($mid);
		 $time = $arr['time']; //最后一次下注的时间戳
     $dosql->Execute("SELECT xiazhu_sum FROM pmw_xiazhuorder where xiazhu_timestamp >= $time and uid=$mid");
		 while($row=$dosql->GetArray()){
			 $sum_money += $row['xiazhu_sum'];
		 }
      return $sum_money;
	 }
	...
	//计算所有的一级下线的数量
	function get_yiji($uid)
	{
		// code...
		global $dosql;

		$r=$dosql->GetOne("SELECT ucode FROM pmw_members where id=$uid");

		$ucode= $r['ucode'];

		$dosql->Execute("SELECT id FROM pmw_members where bcode='$ucode'");
	...
	function get_xiazhusum($starttime,$endtime,$uid)
	{
		// code...
		global $dosql;
   ...
   $r=$dosql->GetOne("SELECT SUM(xiazhu_sum) as summoney FROM pmw_xiazhuorder where uid=$uid and xiazhu_timestamp >= $starttime and xiazhu_timestamp < $endtime");

	 $sum_money= $r['summoney'];
	...
	function get_zhongjiangsum($starttime,$endtime,$uid)
	{
		// code...
		global $dosql;
   ...
   $r=$dosql->GetOne("SELECT SUM(xiazhu_jiangjin) as jiangjin FROM pmw_xiazhuorder where uid=$uid and xiazhu_timestamp >= $starttime and xiazhu_timestamp < $endtime");

	 $jiangjin_money= $r['jiangjin'];

	 return $jiangjin_money;

	}

	//投注提成

	function get_touzhu_ticheng($starttime,$endtime,$uid)
	{
		// code...
		global $dosql;
	 ...
	 $r=$dosql->GetOne("SELECT SUM(money_list) as moneys FROM pmw_record where mid=$uid and types='ticheng' and time_list >= $starttime and time_list < $endtime");
	...
	function get_erji_nums($uid)
	{
		// code...

		global $dosql;

		$dosql->Execute("SELECT id FROM pmw_daili where uid=$uid");
    ...
    global $dosql;
		...
		$dosql->Execute("SELECT * FROM pmw_daili where uid=$uid",$one);
		...
		if($num==0){
			...
			while($row=$dosql->GetArray($one)){
			  $ejcode = $row ['ejcode'];
				$erji_sum=0;
			  $dosql->Execute("SELECT sum(a.xiazhu_sum) as sum FROM pmw_xiazhuorder a inner join pmw_members b on a.uid=b.id where b.ucode=$ejcode",$two);

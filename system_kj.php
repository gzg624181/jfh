<?php
    ...
    $kj_endtime =rtrim($kj_time,"]");

    $kjcodes= ltrim($kjcodes,"[");
    $kjcodes =rtrim($kjcodes,"]");

    $kjcode= ltrim($kjcode,"[");
    $kjcode =rtrim($kjcode,"]");

  // $issue=$current_num;  //当前最新的开奖期数
  // $kj_endtime = $kj_time;
     $kj_endtime_sjc= strtotime($kj_endtime);
     $number_arr= explode(",",$kjcodes);
     $kj_number=$number_arr[0].$number_arr[1].$number_arr[2];//开奖号码
     $kj_he=$kjcode;
     $kj_varchar=$number_arr[0]."+".$number_arr[1]."+".$number_arr[2]."=".$kj_he.results($kj_he);
     $addtime=date("Y-m-d H:i:s");
     $kj_maketime=date("Y-m-d");
     $kj_mdhi=date("m-d H:i:s",$now);
     //更新最新的开奖结果到数据库里面来
     $sql = "UPDATE `$tbname` SET kj_number='$kj_number', kj_varchar='$kj_varchar', kj_he=$kj_he, kj_maketime='$kj_maketime', addtime='$addtime', addtimestamp=$now, kj_state=1, state='kj',kj_endtime='$kj_endtime',kj_endtime_sjc=$kj_endtime_sjc WHERE kj_times=$issue";
   	 $dosql->ExecNoneQuery($sql);

     //===============================================================================================
     //生成一条新的即将开奖的信息
     $kj_endtime_sjc1=strtotime($kj_endtime)+210; //开奖完毕之后，大概的下一期开奖时间为210s之后

     $kj_mdhi=date("m-d H:i:s",$kj_endtime_sjc1);
     $kj_endtime=date("Y-m-d H:i:s",$kj_endtime_sjc1);
     $state='xz'; //当前处于下注状态
     $kj_times=$issue+1;
     $r=$dosql->GetOne("SELECT id from pmw_lotterynumber where kj_times='$kj_times'");
     if(!is_array($r)){
     $sql = "INSERT INTO `pmw_lotterynumber` (kj_times,kj_endtime,kj_endtime_sjc,kj_mdhi,state) VALUES ($kj_times, '$kj_endtime','$kj_endtime_sjc1', '$kj_mdhi', '$state')";
     $dosql->ExecNoneQuery($sql);
     }

    ...
    if($num==3){//选择特码
       if($array[0]>=0 && $array[1]<=27){
        ...
        if(is_numeric($array[0])){  //如果第一个字段是数字，则表示下注的金额
          $type=getkuaitou($array[1]);
          $beilv=get_gamebl($type,getmulu($type),$gameid);
          if($type=="" || $beilv==""){
             $arr=array();
          }else{
          $arr[$key]['money']= intval($array[0]);  //下注的金额
          $arr[$key]['type']=  $type;  //下注的类型(大小单双，大单，小单，大双，小双)
          $arr[$key]['beilv']= get_gamebl($type,getmulu($type),$gameid);
          $arr[$key]['mulu']=  getmulu($type);
          }
        }else{// 第二个字段是数字 大50 小100
          $type=getkuaitou($array[0]);
          $beilv=get_gamebl($type,getmulu($type),$gameid);
          if($type=="" || $beilv==""){
             $arr=array();
          }else{
          $arr[$key]['money']= intval($array[1]);   //下注的金额
          $arr[$key]['type']=  $type;  //下注的类型
          $arr[$key]['beilv']= get_gamebl($type,getmulu($type),$gameid);
          $arr[$key]['mulu']=  getmulu($type);
          }
...
if(isset($token) && $token==$cfg_auth_key){
  ...
  if($imagesurl==""){
  $imagesurl=$cfg_weburl."/templates/default/images/avatar/noimage.jpg";
  }else{
  ...
  $arrg=https_request($url);

  $array=json_decode($arrg,true);
  ...
  if($state==1 || $state==2){
   ...
   // for($i=0;$i<count($arr);$i++){
   //   $xiazhu_type=$arr['type'];
   //   $xiazhu_money=$arr['money'];
   //   $xiazhu_beilv=$arr['beilv'];
   //   $xiazhu_mulu=$arr['mulu'];
   //   $k=$dosql->GetOne("SELECT id FROM `#@__xiazhucontent` WHERE randkey=$randkey");
   //   $id=$k['id'];
   //   $arr['id']=$id;
   // }

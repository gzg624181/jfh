...
window.location.href='lottery.php?keyword='+keyword;
  ...
  function checkinfo(key){
     var v= key;
	// alert(v)
	window.location.href='lottery.php?check='+v;
	}
</script>
<?php
//初始化参数
...
$one=1;
$dosql->Execute("SELECT * FROM `#@__lotterynumber`",$one);
$num=$dosql->GetTotalRow($one);
?>

<div class="topToolbar"> <span class="title">开奖号码数：<span class="num" style="color:red;"><?php echo $num;?></span>
...
<ul>
 <li class="<?php if($check==""){echo "on";}?>"><a href="lottery.php">全部</a></li> <li class="line">-</li>
 <li class="<?php if($check=="on"){echo "on";}?>"><a href="javascript:;" onclick="checkinfo('on')">已开奖</a></li>
 <li class="line">-</li>
 <li class="<?php if($check=="off"){echo "on";}?>"><a href="javascript:;" onclick="checkinfo('off')">未开奖</a></li>
 <li class="line">-</li>
 <li class="<?php if($check=="blank"){echo "on";}?>"><a href="javascript:;" onclick="checkinfo('blank')">空数据</a></li>
	</ul>
...
<form name="form" id="form" method="post" action="lotter_save.php">
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="dataTable">
		...
		$dopage->GetPage("SELECT * FROM `#@__lotterynumber` where kj_state=1");
		}elseif($check=="off"){
		$dopage->GetPage("SELECT * FROM `#@__lotterynumber` where kj_state=0");
		}elseif($check=="blank"){
		$dopage->GetPage("SELECT * FROM `#@__lotterynumber` where  kj_number=''");
		}else{

...
elseif($action == 'gonggao_add')
{
	...
	if($type=="important"){
		$tbname="pmw_important";
		$issuetime=time();
		$sql="UPDATE $tbname set content='$content',faxtime=$issuetime where id=1";
	}else{
	  $tbname="pmw_gonggao";
		$issuetime=time();
...
//删除公告
}elseif($action=="del_gonggao"){
	$sql = "DELETE  from pmw_gonggao where id=$id";
	$dosql->ExecNoneQuery($sql);
	$gourls="gonggao.php";
	header("location:$gourls");
	exit();
